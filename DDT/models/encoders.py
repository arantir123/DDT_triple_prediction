...
# Fully Lorentz GCN based encoder
class HyboNet(Encoder):
    ...
    def __init__(self, c, args, n_nodes, feat_dim, is_add = 0):
        super(HyboNet, self).__init__(c)
        self.manifold = getattr(manifolds, args.manifold)() # hyperbolic operation function
        self.n_nodes = n_nodes
        self.feat_dim = feat_dim
        self.is_add = is_add # args.GCN_add_layers
        assert args.num_layers > 1
        dims, acts, self.curvatures = hyp_layers.get_dim_act_curv(args, self.n_nodes, self.feat_dim, self.is_add) # obtain hyper-parameter for each sub-layer
