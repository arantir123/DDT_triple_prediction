...
from models.base_models import HyboNet4KG_with_pretrained
...
# mainly for hyperbolic based variants using ECFP6 + pre-trained target embeddings from detached1.py or using ECFP6 + target sequence similarity
...
def run_model_predict_DDT(args):
    ...
    hyponet4kg = HyboNet4KG_with_pretrained(target_features_list, processed_adj, all_drug_morgan, abs2_rel_list, args, args.dim, args.max_scale, args.margin)
    ...
    file = './outputs/' + 'Hypo_ECFP6_ontology_ppi_detached2_log_%d.%s.txt' % (args.dim, args.data)
    with open(file, 'a') as f:
        ...
        if args.save_model:
            torch.save(hyponet4kg, './outputs/' + 'Hypo_ECFP6_ontology_ppi_detached2_{}.pt'.format(time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime())))
...
if __name__=='__main__':
    ...
    args.data = 'DTINet'
    args.save_model = True  # whether to save the model to the 'outputs' folder
    args.dim = 128  # overall embedding dimension in the model
    args.epochs = 500  # default: 500, at least 1 epochs
    ...
    args.early_stop = 15
    args.valid_steps = 10  # default: 10, should be smaller than the epoch number
    args.optimizer = 'radam'

    # 2: hyperbolic GCN part
    # *** extra parameter, if set it to True, the below hyper-parameters for processing graphs in part 2 will not be valid, instead, read sequence-like pre-defined target embeddings directly ***
    # [0]: True or False, [1]: 'similarity' (target sequence similarity) or 'pretrained' (pretrained embeddings generated from Hypo_ECFP6_ontology_ppi_detached1.py)
    # [2]: 'expmap' (use expmap function) or 'direct' (directly treat read file as model input)
    args.target_pretrained = [True, 'similarity', 'expmap']
