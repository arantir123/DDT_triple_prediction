...
import manifolds.lmath_ as math
import geoopt
...
class Lorentz(LorentzOri):
    ...
    # expmap0 without extra norm
    def expmap0_without_norm(self, u: torch.Tensor, *, project=True, dim=-1) -> torch.Tensor:
        res = math.expmap0(u, k=self.k, dim=dim)
        # Safe projection on the manifold for numerical stability.
        if project:
            return math.project_without_norm(res, k=self.k, dim=dim)
        else:
            return res
