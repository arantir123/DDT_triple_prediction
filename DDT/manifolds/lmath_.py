...
# this version is for HyboNetnoEncoder

# EXP_MAX_NORM is only used to control the norm in expmap function
# EXP_MAX_NORM = 1.5
EXP_MAX_NORM = 10.0  # for comparison with the complete model using ECFP6 and target graphs, change the norm for expmap to 10.0 at first
print('EXP_MAX_NORM in lmath_:', EXP_MAX_NORM)
...
def _project(x, k: torch.Tensor, dim: int = -1, max_norm: float = 1.5):
    ...
    if max_norm:
        # extra norm based on p2 norm
        # https://blog.csdn.net/blueblood7/article/details/115196258
...
# project function without max_norm
def project_without_norm(x, *, k, dim=-1):
    r"""
    Projection on the Hyperboloid.
    ...
    tensor
        projected vector on the manifold
    """
    return _project_without_norm(x, k=k, dim=dim)


@torch.jit.script
def _project_without_norm(x, k: torch.Tensor, dim: int = -1):
    dn = x.size(dim) - 1
    right_ = x.narrow(dim, 1, dn)
    left_ = torch.sqrt(k + (right_ * right_).sum(dim=dim, keepdim=True))
    x = torch.cat((left_, right_), dim=dim)
...
def _expmap(x, u, k: torch.Tensor, dim: int = -1):
    # nomin = (_norm(u, keepdim=True, dim=dim)).clamp_max(3.)
...
def _logmap(x, y, k, dim: int = -1):
    ...
    # alpha = -1 / k.sqrt() * _inner(x, y, k, keepdim=True)
    # nom = acosh(alpha)
