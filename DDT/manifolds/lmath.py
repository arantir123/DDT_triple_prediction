...
# control the size of norm in expmap and expmap0 functions
EXP_MAX_NORM = 10.0
print('EXP_MAX_NORM in lmath', EXP_MAX_NORM)
...
# project function with extra norm (controlled by max_norm)
def project_with_norm(x, *, k, dim=-1, max_norm=None):
    r"""
    Projection on the Hyperboloid.
    ...
    tensor
        projected vector on the manifold
    """
    return _project_with_norm(x, k=k, dim=dim, max_norm=max_norm if max_norm is not None else 0)


@torch.jit.script
def _project_with_norm(x, k: torch.Tensor, dim: int = -1, max_norm: float = 1.5):
    dn = x.size(dim) - 1
    # obtain spatial dimension
    right_ = x.narrow(dim, 1, dn)
    # re-calculate the time dimension based on spatial dimension
    if max_norm:
        # extra norm
...
def _expmap(x, u, k: torch.Tensor, dim: int = -1):
    ...
    # calculate the Lorentz norm of u (the square of the Lorentz product)
...
# u is the input feature on the tangent space of the origin (n+1 dimension already with the time dimension)
